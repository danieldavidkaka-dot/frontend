...
const translations: Record<Language, Translations> = {
  en: {
    ...
    hero: { 
      ...
      description: 'INVODEX is not a simple transactional bot. It is an advanced Cognitive Runtime and Autonomous Agent designed to execute complex workflows, manage servers (via MCP), process documents via Omnichannel Gateways (WhatsApp/Telegram/Web), and autonomously monitor corporate environments.',
    ...
    capabilities: { label: 'CORE_CAPABILITIES', title_core: 'Key', title_suffix: 'Features', modules_loaded: 'SYSTEMS: NOMINAL', status: 'RBAC: SECURE', items: [
      { title: 'B2B White-Labeling & Stateless Multi-Tenant', desc: 'Designed for immediate SaaS deployment. Through environment variables, the system injects a permanent "Layer 1 Soul" into the LLM. Its Stateless Cognitive Core and Native Multi-Tenant isolation in Supabase guarantee strict data segregation per Client ID, protected by a built-in Anti-DoS Rate Limiting Shield.' },
      { title: 'AutoDream Memory Consolidation', desc: 'Autonomous engine that during low-traffic periods runs background cognitive loops to summarize, consolidate, and compress long-term memory in Supabase. Timezone-aware scheduling prevents UTC cloud drift, maintaining infinite contextual awareness.' },
      { title: 'Unified Gemini Cognitive Core (3.1 Pro)', desc: 'Bypasses intermediary wrappers and connects directly to Google Gemini (gemini-3.1-pro-preview). This native integration guarantees ultra-low latency, flawless native tool calling, and advanced multimodal processing (vision and audio).' },
      { title: 'AIEOS Digital DNA & Cognitive Sandwich', desc: 'Multi-layered Cognitive Sandwich (Soul + DNA + Skills + Working Context). Personas defined via strict JSON contracts physically alter the LLM\'s Temperature and forbidden vocabulary in real-time, switching from creative Growth Hacker (0.85) to robotic OCR (0.00) in milliseconds.' },
      { title: 'Natural Language Gateway (NLP Routing)', desc: 'The Omnichannel Gateway features an NLP filter that cleanses filler words and ignores punctuation. Users can type naturally — "invo, enter dev mode" or simply "invo dev" — and the router seamlessly extracts the core intent.' },
      { title: 'Zero-Trust Architecture (PBAC)', desc: 'Role-Based Access Control handled directly via AIEOS JSON DNA (allowed_tools). If a tool isn\'t in the persona\'s mathematical DNA, it is mathematically impossible to execute. Combined with strict Zod Fail-Fast validation for guaranteed startup integrity.' },
      { title: 'Universal Enterprise ERP Integration', desc: 'Built for B2B accounting automation. Automatically detects the target Odoo version (Modern v13+ or Legacy v12-) and adapts JSON-RPC payloads on the fly. Includes database protection with case-insensitive searches, InvoDex invoice injection, and autonomous CFO financial audits.' },
      { title: 'Self-Healing Auditory Cortex & Link Purifier', desc: 'Native WhatsApp and Telegram audio transcription with indestructible fallback mechanism. Stealth-Mode Puppeteer engine strips DOM trash, avoids Captcha blocks, and feeds pure semantic Markdown to the LLM, reducing token usage by up to 90%.' }
    ] },
    architecture: { label: '// Omnichannel Architecture', title_swarm: 'V5.0', title_intelligence: 'Architecture', description: '> With the V5.0 Omnichannel Architecture, INVODEX has reached a new paradigm: AIEOS (AI Entity Object Specification), Universal Deterministic ERP Integration, Direct Gemini Architecture, and a B2B-Ready White-Labeling structure. It can morph its personality based on mathematical DNA files, execute strict local and remote tools, and defend its corporate identity under an impenetrable "Titanium Cage" security model.', layer1_title: 'Input Layer', layer1: 'Omnichannel Gateways: WhatsApp, Telegram, Web Chat, and HTTP/WS API act as the sensory inputs for document processing and real-time communication.', layer2_title: 'Processing Core', layer2: 'Unified Gemini Cognitive Core (3.1 Pro): Direct integration with Google Gemini processes intents using AIEOS Digital DNA and the Cognitive Sandwich architecture.', layer3_title: 'Execution Layer', layer3: 'Autonomous Execution: Manages MCP tools, ERP integrations (Odoo/InvoDex), and server operations with Zero-Trust PBAC security enforcement.' },
    benchmarks: { 
    ...
    useCases: {
      ...
      items: [
        { title: "Omnichannel Document Processing", description: "INVODEX monitors WhatsApp, Telegram, and Web Chat gateways, extracts document data using Gemini vision models, validates it against corporate records, and automatically posts entries into the ERP.", tags: ["Omnichannel", "Automation", "Vision AI"] },
        { title: "Universal ERP Automation (Odoo)", description: "Automatically detects the target Odoo version (Modern v13+ or Legacy v12-) and adapts JSON-RPC payloads. Employs case-insensitive searches to prevent duplicate vendor creation, with autonomous CFO financial audits.", tags: ["ERP Integration", "Odoo", "Accounting"] },
        { title: "Direct Gemini 3.1 Pro Integration", description: "Bypasses intermediary wrappers and connects directly to Google Gemini (gemini-3.1-pro-preview) for ultra-low latency, flawless native tool calling (Function Declarations), and advanced multimodal processing.", tags: ["Gemini", "Low Latency", "Multimodal"] },
        { title: "Titanium Cage Security", description: "Zero-Trust Architecture (PBAC) reads allowed_tools directly from AIEOS JSON DNA. Combined with Zod Fail-Fast validation, missing credentials prevent startup rather than causing silent runtime failures.", tags: ["Cybersecurity", "Zero-Trust", "AIEOS"] }
      ]
    },
    testimonials: {
    ...
    console: {
      ...
      qr_title: 'WhatsApp Gateway',
      qr_desc: 'Scan this QR code with your WhatsApp app to connect the agent.',
      waiting_qr: 'Generating QR code...',
      secure_conn: 'End-to-end encrypted'
    }
  },
  es: {
    ...
    hero: { 
      ...
      description: 'INVODEX no es un simple bot transaccional. Es un Runtime Cognitivo Avanzado y Agente Autónomo diseñado para ejecutar flujos de trabajo complejos, gestionar servidores (vía MCP), procesar documentos a través de Gateways Omnicanal (WhatsApp/Telegram/Web) y monitorizar entornos corporativos de forma autónoma.',
    ...
    capabilities: { label: 'CAPACIDADES_CORE', title_core: 'Capacidades', title_suffix: 'Principales', modules_loaded: 'SISTEMAS: NOMINAL', status: 'RBAC: SEGURO', items: [
      { title: 'Marca Blanca B2B y Multi-Inquilino Stateless', desc: 'Diseñado para despliegue SaaS inmediato. Mediante variables de entorno, el sistema inyecta un "Alma Capa 1" permanente en el LLM. Su Núcleo Cognitivo Stateless y el aislamiento Multi-Inquilino Nativo en Supabase garantizan segregación estricta por ID de Cliente, protegido por un Escudo Anti-DoS de Rate Limiting.' },
      { title: 'Consolidación de Memoria AutoDream', desc: 'Motor autónomo que durante periodos de bajo tráfico ejecuta bucles cognitivos en segundo plano para resumir, consolidar y comprimir la memoria a largo plazo en Supabase. Programación consciente de zona horaria que previene la deriva UTC de servidores cloud.' },
      { title: 'Núcleo Cognitivo Gemini Unificado (3.1 Pro)', desc: 'Conecta directamente a Google Gemini (gemini-3.1-pro-preview) sin intermediarios. Esta integración nativa garantiza latencia ultra-baja, invocación nativa de herramientas impecable y procesamiento multimodal avanzado (visión y audio).' },
      { title: 'ADN Digital AIEOS y Sándwich Cognitivo', desc: 'Sándwich Cognitivo multicapa (Alma + ADN + Habilidades + Contexto Activo). Las personas definidas vía contratos JSON estrictos alteran físicamente la Temperatura y el vocabulario prohibido del LLM en tiempo real, cambiando de Growth Hacker creativo (0.85) a procesador OCR robótico (0.00) en milisegundos.' },
      { title: 'Gateway de Lenguaje Natural (Enrutamiento NLP)', desc: 'El Gateway Omnicanal incluye un filtro NLP que limpia palabras de relleno e ignora puntuación. Los usuarios pueden escribir naturalmente — "invo, entra en modo dev" o simplemente "invo dev" — y el router extrae la intención central sin esfuerzo.' },
      { title: 'Arquitectura Zero-Trust (PBAC)', desc: 'El Control de Acceso Basado en Roles se gestiona directamente vía ADN JSON de AIEOS (allowed_tools). Si una herramienta no está en el ADN matemático de la persona, es matemáticamente imposible ejecutarla. Combinado con validación Zod Fail-Fast para integridad de arranque garantizada.' },
      { title: 'Integración ERP Empresarial Universal', desc: 'Construido para automatización contable B2B. Detecta automáticamente la versión de Odoo (Moderno v13+ o Legacy v12-) y adapta los payloads JSON-RPC al vuelo. Incluye protección de base de datos con búsquedas insensibles a mayúsculas, inyección de facturas InvoDex y auditorías financieras CFO autónomas.' },
      { title: 'Córtex Auditivo Auto-reparable y Purificador de Enlaces', desc: 'Transcripción de audio nativa de WhatsApp y Telegram con mecanismo de fallback indestructible. Motor Puppeteer en Modo Stealth que elimina basura DOM, evita bloqueos Captcha y alimenta Markdown semántico puro al LLM, reduciendo el uso de tokens hasta un 90%.' }
    ] },
    architecture: { label: '// Arquitectura Omnicanal', title_swarm: 'V5.0', title_intelligence: 'Arquitectura', description: '> Con la Arquitectura Omnicanal V5.0, INVODEX ha alcanzado un nuevo paradigma: AIEOS (AI Entity Object Specification), Integración ERP Determinista Universal, Arquitectura Gemini Directa, y una estructura White-Labeling lista para B2B. Puede transformar su personalidad basándose en archivos de ADN matemático, ejecutar herramientas locales y remotas estrictas, y defender su identidad corporativa bajo un modelo de seguridad impenetrable "Titanium Cage".', layer1_title: 'Capa de Entrada', layer1: 'Gateways Omnicanal: WhatsApp, Telegram, Chat Web y API HTTP/WS actúan como entradas sensoriales para procesamiento de documentos y comunicación en tiempo real.', layer2_title: 'Núcleo de Procesamiento', layer2: 'Núcleo Cognitivo Gemini Unificado (3.1 Pro): Integración directa con Google Gemini procesa intenciones usando ADN Digital AIEOS y la arquitectura de Sándwich Cognitivo.', layer3_title: 'Capa de Ejecución', layer3: 'Ejecución Autónoma: Gestiona herramientas MCP, integraciones ERP (Odoo/InvoDex) y operaciones de servidor con seguridad Zero-Trust PBAC.' },
    benchmarks: { 
    ...
    useCases: {
      ...
      items: [
        { title: "Procesamiento de Documentos Omnicanal", description: "INVODEX monitoriza los gateways de WhatsApp, Telegram y Chat Web, extrae datos de documentos usando modelos de visión Gemini, los valida frente a registros corporativos y publica las entradas automáticamente en el ERP.", tags: ["Omnicanal", "Automatización", "IA de Visión"] },
        { title: "Automatización ERP Universal (Odoo)", description: "Detecta automáticamente la versión de Odoo (Moderno v13+ o Legacy v12-) y adapta los payloads JSON-RPC. Emplea búsquedas insensibles a mayúsculas para prevenir creación de proveedores duplicados, con auditorías financieras CFO autónomas.", tags: ["Integración ERP", "Odoo", "Contabilidad"] },
        { title: "Integración Directa Gemini 3.1 Pro", description: "Conecta directamente a Google Gemini (gemini-3.1-pro-preview) sin intermediarios para latencia ultra-baja, invocación nativa de herramientas impecable (Function Declarations) y procesamiento multimodal avanzado.", tags: ["Gemini", "Baja Latencia", "Multimodal"] },
        { title: "Seguridad Titanium Cage", description: "La Arquitectura Zero-Trust (PBAC) lee allowed_tools directamente del ADN JSON de AIEOS. Combinado con validación Zod Fail-Fast, las credenciales faltantes impiden el arranque en lugar de causar fallos silenciosos en tiempo de ejecución.", tags: ["Ciberseguridad", "Zero-Trust", "AIEOS"] }
      ]
    },
    testimonials: {
    ...
    console: {
      ...
      status_booting: 'Iniciando...',
      status_offline: 'Desconectado',
      logs_title: 'Registros del Sistema',
      qr_title: 'Puerta de Enlace WhatsApp',
      qr_desc: 'Escanea este código QR con tu aplicación de WhatsApp para conectar el agente.',
      waiting_qr: 'Generando código QR...',
      secure_conn: 'Cifrado de extremo a extremo'
    }
  },
